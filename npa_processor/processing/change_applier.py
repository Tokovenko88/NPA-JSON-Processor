...
from npa_processor.processing.html_utils import _correct_table_highlights
...
def apply_grouped_changes(element, changes, valid_from, change_data, data, model, prompt4,
    ...
    if text_changes:
        ...
        for ch in text_changes:
            ...
            answer = ask_ollama(stage4_prompt, model, log_callback, extra_options, stop_event, change_info=desc)
            if not answer:
                if log_callback:
    ...
    for op in pending_paragraph_ops:
        ...
        if op_type == 'new_redaction':
            ...
            if 1 <= target_idx <= len(ai_paragraphs):
                ...
                answer = ask_ollama(stage4_prompt, model, log_callback, extra_options, stop_event, change_info=original_op.get('description', ''))
                if answer:
...
def apply_change(change, data, change_data, law_ref, general_valid_from, log_callback,
    ...
    if '_resolved_item_id' in change:
        ...
        if resolved_target_id == '__наименование__':
            ...
            return _apply_change_to_element_content(target_element, ch_type, description, valid_from, modified_by_id_str, model, prompt4, extra_options, stop_event, log_callback, rebuild_ids, structural, source_context_root, change_data, data, None, source_item_id, rev_number, manual_resolver)
    structural = change.get('structural_element', '').strip()
    ch_type = change.get('type', '').strip()
    description = change.get('description', '')
    rev_number = change.get('revision_number', None)
    ...
    if not modified_by_id_str:
        modified_by_id_str = str(change_data.get('npa_id', 'unknown'))
    return _apply_change_to_element_content(target_element, ch_type, description, valid_from, modified_by_id_str, model, prompt4, extra_options, stop_event, log_callback, rebuild_ids, structural, source_context_root, change_data, data, source_element, source_item_id, rev_number, manual_resolver)
...
def _apply_change_to_appendix_prefix(change, data, change_data, valid_from, rev_number,
    ...
    if ch_type == 'new_redaction':
        ...
        answer = ask_ollama(stage4_prompt, model, log_callback, extra_options, stop_event, change_info=description)
        if answer:
...
def _apply_change_to_head(change, data, change_data, valid_from, rev_number,
    ...
    if ch_type == 'change':
        ...
        answer = ask_ollama(stage4_prompt, model, log_callback, extra_options, stop_event, change_info=change.get('description', ''))
        if answer is None:
            if log_callback:
...
def _apply_change_to_element_head(change, data, change_data, valid_from, rev_number,
    ...
    if ch_type == 'new_redaction':
        ...
        answer_head = ask_ollama(stage4_prompt, model, log_callback, extra_options, stop_event, change_info=change.get('description', ''))
